        ...
        request(getScheduleUrl('basketball'), (basketballError, basketballResponse, basketballBody: string) => {
            ...
            if (mostRecentEvent) {
                ...
                if (us.score) {
                    ...
                    // Game is currently happening, so we need to make a separate request
                    // since the schedule doesn't have current scores
                    status = 'NOT YET';
                    request(
                        getCurrentGameUrl(
                        ...
                        (err, response, body: string) => {
                            ...
                            if (!err && response.statusCode === 200) {
                                ...
                                us = event.competitions[0].competitors.find(
                                    (competitor) => competitor.id === ESPN_TEAM_ID
                                );
                                score = +us.score;
